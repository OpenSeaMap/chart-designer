...
import osmcbdef.exceptions.MapDownloadSkippedException;
import osmcbdef.gui.AtlasProgress;
import osmcbdef.gui.AtlasProgress.AtlasCreationController;
import osmcbdef.program.atlascreators.AtlasCreator;
import osmcbdef.program.atlascreators.tileprovider.DownloadedTileProvider;
import osmcbdef.program.atlascreators.tileprovider.FilteredMapSourceProvider;
import osmcbdef.program.atlascreators.tileprovider.TileProvider;
import osmcbdef.program.download.DownloadJobProducerThread;
import osmcbdef.program.interfaces.AtlasInterface;
import osmcbdef.program.interfaces.DownloadJobListener;
import osmcbdef.program.interfaces.DownloadableElement;
...
import osmcbdef.program.model.AtlasOutputFormat;
...
import osmcbdef.utilities.tar.TarIndex;
import osmcbdef.utilities.tar.TarIndexedArchive;
...
public class AtlasThread extends Thread implements DownloadJobListener, AtlasCreationController {
	...
	private boolean quitOsmcbAfterAtlasCreation = false;
	...
	public void run() {
		...
		try {
			...
			if (quitOsmcbAfterAtlasCreation)
				System.exit(0);
		...
		try {
			...
			if (!(map.getMapSource() instanceof FileBasedMapSource)) {
				...
				if (!AtlasOutputFormat.TILESTORE.equals(atlas.getOutputFormat())) {
					String tempSuffix = "OSMCB_" + atlas.getName() + "_" + zoom + "_";
	...
	public void setQuitOsmcbAfterAtlasCreation(boolean quitOsmcbAfterAtlasCreation) {
		this.quitOsmcbAfterAtlasCreation = quitOsmcbAfterAtlasCreation;
	}
