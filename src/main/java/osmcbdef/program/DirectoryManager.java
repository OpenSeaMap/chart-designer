...
/**
 * Provides the common directories used within OpenSeaMap ChartBundler:
 * <ul>
...
public class DirectoryManager {
	...
	static {
		...
		tempDir = applyDirConfig("osmcbdef.tmpdir", new File(System.getProperty("java.io.tmpdir")));

		mapSourcesDir = applyDirConfig("osmcbdef.mapsourcesdir", new File(programDir, "mapsources"));
		toolsDir = applyDirConfig("osmcbdef.toolsdir", new File(programDir, "tools"));
		userSettingsDir = applyDirConfig("osmcbdef.usersettingsdir", programDir);
		atlasProfilesDir = applyDirConfig("osmcbdef.atlasprofilesdir", currentDir);
		tileStoreDir = applyDirConfig("osmcbdef.tilestoredir", new File(programDir, "tilestore"));
	}
	...
	private static File expandCommandLine(final String cmd) {
		...
		while (m.find()) {
			...
			if (envVal == null) {
				...
				if ("osmcbdef-prog".equalsIgnoreCase(envVar))
					defPath = programDir;
	...
	/**
	 * Returns the directory where OpenSeaMap ChartBundler saves it's application settings.
	 * 
	 * Examples:
	 * <ul>
	 ...
	 * <li>Linux:<br>
	 * <tt>/home/$username$/.osmcbdef</tt></li>
	 * </ul>
	 * 
	 * @return
	 */
	private static File getUserAppDataDir() {
		...
		File osmcbUserDir = new File(userDir, ".osmcbdef");
		if (!osmcbUserDir.exists() && !osmcbUserDir.mkdir())
			throw new RuntimeException("Unable to create directory \"" + osmcbUserDir.getAbsolutePath() + "\"");
		return osmcbUserDir;
	}
}
