...
//	OSMCDStrs.RStr("Catalog.Confirm");
//	String.format(OSMCDStrs.RStr("Catalog.ConfirmOverwrite.Existing.Mismatching"), fileSel.getPath(), newCatalogsName, Catalog.getCatalogFileName(newCatalogsName), catalogsDir);
//	OSMCDStrs.RStr("Catalog.Confirm");
//	
...
//	
	...
	protected String makeOptionDlgMsg()
	{
		if (existingPath == null) // new
			return String.format(OSMCDStrs.RStr("Catalog.ConfirmOverwrite.New"),
					chosenName, Catalog.getCatalogFileName(chosenName), strCatalogsDir);
		// else: existing file
		if (conformLatinName == null)
			return String.format(OSMCDStrs.RStr("Catalog.ConfirmOverwrite.Existing.Mismatching"),
					existingPath, chosenName, Catalog.getCatalogFileName(chosenName), strCatalogsDir);
		else
			return String.format(OSMCDStrs.RStr("Catalog.ConfirmOverwrite.Existing"),
					chosenName, existingPath, Catalog.getCatalogFileName(chosenName), strCatalogsDir);
	}
	...
	public void actionPerformed(ActionEvent e)
	{
		if (conformLatinName == null)
		{
			...
			Object[] array = {dlgMessage, OSMCDStrs.RStr("Catalog.RenameField.Info"), catalogNameField};
    	...
    	if (Catalog.isCatalogsFileNamePart(chosenName))
    	{
				...
				int response = JOptionPane.showOptionDialog(parentComponent,
						makeOptionDlgMsg(),
						OSMCDStrs.RStr("Catalog.Confirm"),
