...
public class JTileImageParametersPanel extends JCollapsiblePanel
{
	...
	{
		super(OSMCDStrs.RStr("lp_tile_param_title"), new GridBagLayout());
		setName("TileImageParameters");

		enableCustomTileProcessingCheckButton = new JCheckBox(OSMCDStrs.RStr("lp_tile_param_recreate_checkbox_title"));
		enableCustomTileProcessingCheckButton.addActionListener(new EnableCustomTileSizeCheckButtonListener());
		enableCustomTileProcessingCheckButton.setToolTipText(OSMCDStrs.RStr("lp_tile_param_recreate_checkbox_tips"));

		tileSizeWidthLabel = new JLabel(OSMCDStrs.RStr("lp_tile_param_width_title"));
		tileSizeWidth = new JTileSizeCombo();
		tileSizeWidth.setToolTipText(OSMCDStrs.RStr("lp_tile_param_width_tips"));

		tileSizeHeightLabel = new JLabel(OSMCDStrs.RStr("lp_tile_param_height_title"));
		tileSizeHeight = new JTileSizeCombo();
		tileSizeHeight.setToolTipText(OSMCDStrs.RStr("lp_tile_param_height_tips"));

		tileImageFormatLabel = new JLabel(OSMCDStrs.RStr("lp_tile_param_image_fmt_title"));
	...
	public TileImageParameters getSelectedTileImageParameters()
	{
		...
		if (customTileSize)
		{
			...
			TileImageFormat format = (TileImageFormat) tileImageFormat.getSelectedItem();
	...
	public String getValidationErrorMessages()
	{
		...
		if (!tileSizeHeight.isValueValid())
			errorText += String.format(OSMCDStrs.RStr("lp_tile_param_msg_valid_height"), JTileSizeCombo.MIN, JTileSizeCombo.MAX);

		if (!tileSizeWidth.isValueValid())
			errorText += String.format(OSMCDStrs.RStr("lp_tile_param_msg_valid_width"), JTileSizeCombo.MIN, JTileSizeCombo.MAX);
		return errorText;
	}
