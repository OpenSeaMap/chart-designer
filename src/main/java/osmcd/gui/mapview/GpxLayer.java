...
public class GpxLayer implements IfMapLayer
{
	...
	public void paint(JMapViewer map, Graphics2D g, int zoom, int minX, int minY, int maxX, int maxY)
	{
		...
		if (showWaypoints)
		{
			...
			{
				paintPoint(pt, wptPointColor, g, showWaypointName, zoom, minX, minY, maxX, maxY);
			}
		}
		if (showTracks)
		{
			for (TrkType trk : gpx.getTrk())
			{
				for (TrksegType seg : trk.getTrkseg())
				{
					...
					{
						paintTrack(pt, trkPointColor, g, zoom, minX, minY, maxX, maxY);
					}
				}
			}
		}
		if (showRoutes)
		{
			for (RteType rte : gpx.getRte())
			{
				...
				{
					paintTrack(pt, rtePointColor, g, zoom, minX, minY, maxX, maxY);
				}
			}
		}
	}

	private boolean paintPoint(final WptType point, Color color, final Graphics2D g, boolean paintPointName, int zoom, int minX, int minY, int maxX, int maxY)
	{
		int x = MP2MapSpace.cLonToXIndex(point.getLon().doubleValue(), zoom);
		if (x < minX || x > maxX)
			return false; // Point outside of visible region
		int y = MP2MapSpace.cLatToYIndex(point.getLat().doubleValue(), zoom);
		if (y < minY || y > maxY)
			return false; // Point outside of visible region
		x -= minX;
		y -= minY;
	...
	private boolean paintTrack(final WptType point, Color color, final Graphics2D g, int zoom, int minX, int minY, int maxX, int maxY)
	{
		...
		int xAbs = MP2MapSpace.cLonToXIndex(point.getLon().doubleValue(), zoom);
		int yAbs = MP2MapSpace.cLatToYIndex(point.getLat().doubleValue(), zoom);
