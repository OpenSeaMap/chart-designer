...
import osmcb.program.WgsGrid;
import osmcb.program.abstracts.ACApp;
import osmcb.program.interfaces.IfMapSource;
import osmcb.program.interfaces.IfMapSourceTextAttribution;
...
import osmcd.OSMCDStrs;
import osmcd.gui.mapview.controller.DefaultMapController;
import osmcd.gui.mapview.controller.JMapController;
import osmcd.gui.mapview.controller.MapKeyboardController;
...
public class PreviewMap extends JMapViewer
{
	private static final long serialVersionUID = 1L;
	public static final Color GRID_COLOR = new Color(200, 20, 20, 130);
	public static final Color SEL_COLOR = new Color(0.9f, 0.7f, 0.7f, 0.3f);
	public static final Color MAP_COLOR = new Color(1.0f, 0.84f, 0.0f, 0.07f);
	public static final int MAP_CONTROLLER_RECTANGLE_SELECT = 0;
	public static final int MAP_CONTROLLER_GPX = 1;
	protected static final Font LOADING_FONT = new Font("Sans Serif", Font.BOLD, 30);
	private static Logger log = Logger.getLogger(PreviewMap.class);
	...
	private final WgsGrid wgsGrid = new WgsGrid(OSMCDSettings.getInstance().wgsGrid, this);
	...
	public void settingsSave()
	{
		OSMCDSettings settings = (OSMCDSettings) ACApp.getApp().getSettings();
	 ...
	 * Sets the current view by the current values from {@link OSMCDSettings}
	 * 
	 * 20141212 AH does only work, if it was not copied from somewhere else. implement some
	 * check against that
	 * e.g. selection may be invalid
	 * 
	 */
	public void settingsLoad()
	{
		OSMCDSettings settings = (OSMCDSettings) ACApp.getApp().getSettings();
		IfMapSource mapSource = ACMapSourcesManager.getInstance().getSourceByName(settings.mapviewMapSource);
		if (mapSource != null)
			setMapSource(mapSource);
		...
		setSelectionByTileCoordinate(mapSource.getMaxZoom(), settings.mapviewSelectionMin, settings.mapviewSelectionMax, true);
	}
	...
	protected void paintComponent(Graphics graphics)
	{
		...
		if (((OSMCDSettings) ACApp.getApp().getSettings()).wgsGrid.enabled)
		{
			wgsGrid.paintWgsGrid(g, mapSource.getMapSpace(), tlc, zoom);
		}
		ScaleBar.paintScaleBar(this, g, mapSource.getMapSpace(), tlc, zoom);
