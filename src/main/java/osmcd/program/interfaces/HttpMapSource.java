...
public interface HttpMapSource extends MapSource
{
	 ...
	 * 
	 * <ul>
	 * <li>{@link #IfNoneMatch} Server provides ETag header entry for all tiles and <b>supports</b> conditional download via <code>If-None-Match</code> header
	 * entry.</li>
	 * <li>{@link #ETag} Server provides ETag header entry for all tiles but <b>does not support</b> conditional download via <code>If-None-Match</code> header
	 * entry.</li>
	 * <li>{@link #IfModifiedSince} Server provides Last-Modified header entry for all tiles and <b>supports</b> conditional download via
	 * <code>If-Modified-Since</code> header entry.</li>
	 * <li>{@link #LastModified} Server provides Last-Modified header entry for all tiles but <b>does not support</b> conditional download via
	 * <code>If-Modified-Since</code> header entry.</li>
	...
	/**
	 * Constructs the tile url connection. If necessary the url connection can be prepared with cookies or other http specific headers which are required by the
	 * http server.
	 * 
	 * @param zoom
	 * @param tilex
