 ...
 ******************************************************************************/
package osmcd.program.tilestore.berkeleydb;
...
import osmcd.program.tilestore.berkeleydb.TileDbEntry.TileDbKey;
import osmcd.utilities.GUIExceptionHandler;
import osmcd.utilities.Utilities;
import osmcd.utilities.file.DeleteFileFilter;
import osmcd.utilities.file.DirInfoFileFilter;
...
public class BerkeleyDbTileStore extends TileStore
{
	...
	public BerkeleyDbTileStore() throws TileStoreException {
		...
		String oldPackage4 = "mobac.program.tilestore.berkeleydb";
		String oldPackage5 = "osmcb.program.tilestore.berkeleydb";
		...
		mutations.addRenamer(new Renamer(oldPackage4 + entry, 3, TileDbEntry.class.getName()));
		mutations.addRenamer(new Renamer(oldPackage4 + key, 3, TileDbKey.class.getName()));
		mutations.addRenamer(new Renamer(oldPackage5 + entry, 4, TileDbEntry.class.getName()));
		mutations.addRenamer(new Renamer(oldPackage5 + key, 4, TileDbKey.class.getName()));
	...
	protected void acquireTileStoreLock() throws TileStoreException
	{
		try
		{
			...
			if (tileStoreLock == null)
				throw new TileStoreException("Unable to obtain tile store lock - " + "another instance of OpenSeaMap ChartDesigner is running!");
	...
	@Override
	public void putTileData(byte[] tileData, int x, int y, int zoom, MapSource mapSource, long timeLastModified, long timeExpires, String eTag)
			throws IOException
	{
	...
	/**
	 * Returns the directory used for storing the tile database of the {@link MapSource} specified by <code>mapSource</code>
	 * 
	 * @param mapSource
	 * @return
	 */
